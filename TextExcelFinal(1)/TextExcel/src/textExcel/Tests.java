...
public class Tests {
	...
	public static class Checkpoint2
	{
		...
		public void testProcessCommandSpecialStrings()
		{
			...
			String inspectedSpecial1 = grid.getCell(new Location(6,1)).fullCellText();
			assertEquals("inspected first special string", "\"" + stringSpecial1 + "\"", inspectedSpecial1);
			String inspectedSpecial2 = grid.getCell(new Location(12,5)).fullCellText();
		...
		public void testProcessCommandMoreSpecialStrings()
		{
			...
			for (int col = 0; col < specialStrings.length; col++)
			{
				for (int row = 5; row < 20; row += 10)
				{
					...
					String inspected = grid.getCell(new Location(row, col)).fullCellText();
	...
	public static class ExtraCreditOperationOrder
	{
		...
		public void testSimplePrecedence()
		{
			...
			grid.processCommand("A1 = " + formula);
			String result = grid.getCell(new Location(0, 0)).abbreviatedCellText();
		...
		public void testComplexPrecedence()
		{
			...
			grid.processCommand("L20 = " + formula);
			String result = grid.getCell(new Location(19, 11)).abbreviatedCellText();
		...
		public void testReferencePrecedence()
		{
			...
			String result = grid.getCell(new Location(2, 0)).abbreviatedCellText();
	...
	public static class ExtraCreditSaveLoad
	{
		// Tests for saving+loading extra credit
		Spreadsheet grid;
		...
		public void testSaveLoad()
		{
			Method save = null;
			Method load = null;
			Spreadsheet restoredGrid = new Spreadsheet();
			
			grid.processCommand("A1 = 3");
			grid.processCommand("A2 = ( A1 + 1 )");
			grid.processCommand("A3 = \"string\"");
			grid.processCommand("C1 = 11/22/2000");
			
			try {
				// Use reflection in case students have not implemented
				// these methods.
				// (In real life, one would probably use the Serializable
				// interface for saving and loading, but we haven't covered
				// that in class.)
				save = grid.getClass().getMethod("save", String.class);
				load = grid.getClass().getMethod("load", String.class);
			} catch (NoSuchMethodException e) {
				fail("save and load methods not present");
			}
			
			try {
				save.invoke(grid, "spreadsheet.dat");
				load.invoke(restoredGrid, "spreadsheet.dat");
			} catch (Exception e) {
				fail("couldn't invoke save and load methods");
			}

			new java.io.File("spreadsheet.dat").delete();

			assertEquals("grid restored incorrectly", grid.getGridText(), restoredGrid.getGridText());
			grid.processCommand("A1 = 4");
			restoredGrid.processCommand("A1 = 4");
			assertEquals("restored grid misbehaving", grid.getGridText(), restoredGrid.getGridText());
		}
	}	
}
